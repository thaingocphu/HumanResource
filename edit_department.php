<?php
  ...
  if($_SERVER["REQUEST_METHOD"] == "POST"){
    if(isset($_POST["id"]) && isset($_POST["name"])){
      ...
      $slqi = "SELECT * FROM department WHERE name = '$name' and id_department <> '$id_d' ";
      ...
      {
        $sql = "UPDATE department SET name = '$name' where id_department = '$id_d' ";
        if ($conn->query($sql) === TRUE) {
          $mess = 'Update Department successful!';

          $db_name =$name;
          $db_id = $id_d;

        } else {
            $error = "Update department failed!";
        }
        $conn->close();
      }
    }
  ...
  if($_SERVER["REQUEST_METHOD"] == "GET"){
    ...
    $sql = "SELECT * FROM department WHERE id_department = $id_d ";
                ...
                <div class="panel-body">
                  <form method="POST">
                    <div class="form-group">
                      ...
                      <input value="<?= $db_id ?>" type="text" class="form-control" id="id" placeholder="Enter department's ID" name="id" readonly>
                    </div>
                    <div class="form-group">
                      <label for="name">Department's Name:</label>
                      <input value="<?= $db_name ?>" type="text" class="form-control" id="name" placeholder="Enter department's Name" name="name">
                    </div>
                    <?php
                      if (!empty($error)) {
